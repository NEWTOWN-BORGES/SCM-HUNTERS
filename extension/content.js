 ...
 * CACHE-BUST: 2026-02-10T21:45 - VERSÃO DEBUG UI PERSISTENCE
 */
...
// REMOVIDO: UIModule.init() movido para App.init() para garantir que módulos estão carregados

// Verifica se o CSS foi injetado (Log informativo apenas)
console.log('SCM HUNTERS: Inicializando...');

// AUTO-REPAIR INLINE: Corrige contagens de votos duplicadas (executa imediatamente)
try {
  console.log('[Anti-Scam] 🔧 Auto-Repair: Verificando integridade dos votos locais...');
  Object.keys(localStorage).forEach(key => {
    if (key.startsWith('as_vote_count_')) {
      const hash = key.replace('as_vote_count_', '');
      const votesKey = `as_user_votes_${hash}`;
      const votes = JSON.parse(localStorage.getItem(votesKey) || '{}');
      const realCount = Object.keys(votes).length;
      const savedCount = parseInt(localStorage.getItem(key) || '0');

      if (savedCount > realCount) {
        console.log(`[Anti-Scam] 🔧 Reparando contagem para ${hash}: ${savedCount} -> ${realCount}`);
        localStorage.setItem(key, realCount);
      }
    }
  });
  console.log('[Anti-Scam] ✅ Auto-Repair concluído.');
} catch (e) {
  console.error('[Anti-Scam] ❌ Erro no Auto-Repair:', e);
}
...
const App = {
  ...
  init() {
    ...
    console.log('[Anti-Scam] ✅ v5.3 - SYNCED FIXED + UI PERSISTENCE (2026-02-10)');
    ...
    if (this.currentConfig) {
      ...
      // Inicia UIModule (Auto-repair de contagens de votos)
      if (window.UIModule && window.UIModule.init) {
        console.log('[Anti-Scam] ⚙️ Chamando UIModule.init()...');
        window.UIModule.init();
      } else {
        console.warn('[Anti-Scam] ❌ UIModule.init() não disponível!', {
          hasUIModule: !!window.UIModule,
          hasInit: !!(window.UIModule && window.UIModule.init)
        });
      }
      ...
      const tooltipOptions = {
        ...
        onReport: async (signalType, delta = 1, context = null) => {
          ...
          tooltipOptions.voteQueue = tooltipOptions.voteQueue.then(async () => {
            ...
            // FORCE INTEGER FOR LIKES/DISLIKES (User Request: "sistema de like é diferente")
            if (['votes_like', 'votes_dislike'].includes(signalType)) {
              weight = 1.0; // Sempre 1, sem pesos decimais
            }
